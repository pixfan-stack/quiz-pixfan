...
/**
 * GET /api/leaderboard?limit=10&quizId=exposure-basics
 * Returns top scores (optional filter by quizId)
 */
export const onRequestGetLeaderboard: PagesFunction<Env> = async (context) => {
  const url = new URL(context.request.url);
  const quizId = url.searchParams.get('quizId');
  ...
  try {
    if (context.env.DB) {
      ...
      query += ' ORDER BY percentage DESC LIMIT ?';
      params.push(limit);

      const rows = await context.env.DB.prepare(query).bind(...params).all<HighScoreRecord>();
